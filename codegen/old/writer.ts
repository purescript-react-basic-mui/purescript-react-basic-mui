...
export const componentName = (name: string): string => "_" + lowerCaseFirstLetter(name)
export const functionName = (name: string): string => lowerCaseFirstLetter(name)
...
const writeComponentProps = (props: Props): WrittenProps => {
  const optionalFields = props.fields.filter((field) => field.isOptional || field.fieldType.isOptional).filter(field => field.name !== "children")
  const requiredFields = props.fields.filter((field) => !field.isOptional).filter(field => field.name !== "children")

  const functionBody = (name: string) =>  `${functionName(name)} props = element ${componentName(name)} props`
  ...
  return { fns, props: propsStrs, foreignData: collectForeignData(props.fields), fileName: props.fileName }
}
...
export const writeOtherProps = (props: Props): WrittenProps => {
  ...
  if(props.fields.length === 0){
    ...
    if(props.parents[0] && props.parents[0].types[0] && props.parents[0].name === "NativeSyntheticEvent"){
      ...
      return { fns: [], props: [prop], foreignData: [], fileName: props.fileName }
    } else {
  ...
  return {fns: [], props: [record], foreignData: collectForeignData(props.fields), fileName: props.fileName}
}
...
export const top = (moduleName: string) =>
`-- | ----------------------------------------
...
import React.Basic (Component, JSX, ReactComponent, Ref, element)
